...
const MiniPost = ({ post, reply }) => {
  ...
  return (
    <div className="minipost" onClick={handleClick}>
      <div className="postHeader">
        ...
        >
          <img
            src={
              post.userId.avatar
                ? `${API_URL}/media/avatar/${post.userId._id}`
                : "/default_pfp.svg"
            }
            alt="Profile picture"
          />
      ...
      <div className="postBody">
        ...
        <p>{post.text}</p>
        {post.image && (
          <img onClick={handleOpenImg} src={`${API_URL}/media/${post._id}`} />
        )}
      </div>
      <div className="postButtons">
        <span>
          {React.createElement(liked ? HeartFilled : HeartOutlined, {
            onClick: like,
            className: `icon icon-like ${liked && "liked"}`,
          })}{" "}
          {post.likes.length}
        </span>
